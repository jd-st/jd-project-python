...
class TestPetst0re:
    ...
    def test_method_list_inventory(self, client: JdProject) -> None:
        petst0re = client.petst0re.list_inventory()
        assert_matches_type(Petst0reListInventoryResponse, petst0re, path=["response"])
...
class TestAsyncPetst0re:
    ...
    async def test_method_list_inventory(self, async_client: AsyncJdProject) -> None:
        petst0re = await async_client.petst0re.list_inventory()
        assert_matches_type(Petst0reListInventoryResponse, petst0re, path=["response"])

    @pytest.mark.skip(reason="Prism tests are disabled")
    @parametrize
    async def test_raw_response_list_inventory(self, async_client: AsyncJdProject) -> None:
        response = await async_client.petst0re.with_raw_response.list_inventory()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        petst0re = await response.parse()
        assert_matches_type(Petst0reListInventoryResponse, petst0re, path=["response"])

    @pytest.mark.skip(reason="Prism tests are disabled")
    @parametrize
    async def test_streaming_response_list_inventory(self, async_client: AsyncJdProject) -> None:
        async with async_client.petst0re.with_streaming_response.list_inventory() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
